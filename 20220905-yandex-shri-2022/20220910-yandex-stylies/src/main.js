...
export default (html, css) => {
  ...
  const matchCssToHtml = (node, parentTag=[]) => {
    if (node.type === "ELEMENT" && parentTag.length > 0) {
      parentTag.forEach((parent) =>
        Object.keys(styles[parent]).forEach(
          (prop) => (node.styles[prop] = styles[parent][prop])
        )
      );
    }

    if (node.type === "ELEMENT") {
      Object.keys(styles[node.tag]).forEach(
        (prop) => (node.styles[prop] = styles[node.tag][prop])
      )
    }
